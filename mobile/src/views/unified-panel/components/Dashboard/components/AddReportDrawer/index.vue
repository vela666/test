      ...
      <div class="nd-report-l">
        ...
        <div class="nd-grid-drawer-container">
          <GridLayout
            ...
            v-model="unifiedPanelStore.addSelectedReporNoteList">
          ...
          <el-tab-pane
            ...
            <div class="flex-column gap16">
              <el-select
                ...
                v-model="unifiedPanelStore.selectAppId">
                <el-option
                  v-for="item of unifiedPanelStore.appList"
                  :key="item.appId"
                  :value="item.appId"
                  :label="item.appName" />
              </el-select>
              ...
              <div class="flex gap20">
                ...
                <el-select
                  ...
                  multiple>
                  <el-option
                    v-for="(item, index) of unifiedPanelStore.labelList"
                    :key="index"
                    :label="item"
                    :value="item" />
                </el-select>
...
const reportAndNoteLen = computed(() => {
  return unifiedPanelStore.addSelectedReporNoteList.reduce(
    (p, c) => {
      if (c.type === 1) {
        // 便签
        p.note += 1
...
const filterReportList = computed(() => {
  const businessIds = unifiedPanelStore.addSelectedReporNoteList.map(
    (v) => v.businessId
  )
  return unifiedPanelStore.reportList.filter(
    (item) => !businessIds.includes(item.businessId) && item.reportType !== 5
  )
})

const filterNoteList = computed(() => {
  const addSelectedReporNoteList =
    unifiedPanelStore.addSelectedReporNoteList.map((v) => v.businessId)
  return unifiedPanelStore.noteList.filter(
    (item) =>
...
const appChange = () => {
  state.reportFilter.labelNameList = []
  unifiedPanelStore.asyncGetTagList()
  getReportList()
}
...
const updNote = (val) => {
  ...
  const data = unifiedPanelStore.addSelectedReporNoteList.find(
...
const addSelectedListItem = (data) => {
  ...
  unifiedPanelStore.addSelectedReporNoteList.push(data)
  gridLayoutRef.value.makeLayout(data.mark)
}

const getReportList = async (exec = true) => {
  if (!unifiedPanelStore.selectAppId) return
  exec && (state.loading = true)
  await unifiedPanelStore
    .asyncGetReportList({
...
const getNoteList = async (exec = true) => {
  exec && (state.loading = true)
  await unifiedPanelStore.asyncGetNoteList().finally((_) => {
    tabChange()
    exec && (state.loading = false)
  })
}
...
const open = async () => {
  ...
  await unifiedPanelStore.getSelectableApp()
  await Promise.allSettled([
    unifiedPanelStore.getReportNoteDetails(false),
    getReportList(false),
    getNoteList(false),
    unifiedPanelStore.asyncGetTagList(),
  ]).finally((_) => {
