<template>
  <CommonDrawer
    ...
    size="885px"
    :title="$t('dataManagement.addNewEvent')">
    <CommonUpload
      ref="commonUploadRef"
...
import { asyncAddEvent } from '@/api/modules/programme/event'
...
const submit = () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    state.operateLoading = true
    const { code } = await asyncAddEvent({
      eventList: state.formData.eventList,
    }).finally((_) => {
      state.operateLoading = false
    })
    if (code === 200) {
