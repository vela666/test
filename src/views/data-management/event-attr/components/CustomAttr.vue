<template>
  <CommonDrawer
    ...
    :title="$t('dataManagement.eventAttr.addCustomAttributes')"
...
<script setup>
import { nextTick, markRaw, reactive, ref, h } from 'vue'
...
const handelUploadFile = async (file, fileList) => {
  ...
  state.formData.attrList.forEach((item) => {
    ...
    if (!Array.isArray(item.eventIdList) && +props.type === 1) {
      const idList = item.eventIdList.split(',')
      item.eventIdList = state.configEventList.reduce((p, c) => {
        idList.forEach((item) => {
          if (c.eventName === item) {
            p.push(c.eventId)
          }
...
const getAppEvents = async () => {
  ...
  // 去除预置事件
  state.configEventList = data.filter((item) => item.eventType !== 2)
}
