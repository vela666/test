<template>
  <CommonDrawer
    ...
    :title="`${state.formData.id ? $t('user.userTag.editConditionTag') : $t('user.userTag.addConditionTag')}`"
    size="900px">
    <el-form label-position="top" ref="formRef" :model="state.formData">
      ...
      <LabelValueConfig ref="labelValueConfigRef" v-model="state.formData.qp" />
...
import LabelValueConfig from './LabelValueConfig.vue'
import {
  asyncAddTag,
  asyncEditTag,
  asyncGetByIdTagInfo,
...
const labelValueConfigRef = ref(null)
...
const submit = () => {
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      try {
        const val = await labelValueConfigRef.value.getResult()
...
const getByIdTagInfo = async (params) => {
  ...
  Object.keys(data).forEach((k) => {
    if (Reflect.has(state.formData, k)) {
      if (k === 'qp') {
        state.formData[k] = JSON.parse(data[k]).map((item) => {
          return {
            ...item,
            edit: false,
          }
...
const open = async (data) => {
  ...
  if (data) {
    ...
    nextTick(() => {
      labelValueConfigRef.value.addLabelValue()
    })
  }
}
