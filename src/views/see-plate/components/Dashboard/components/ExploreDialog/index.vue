...
const open = async (val, info) => {
  ...
  /*let eventNames = []
  // getIndexEventName(qp.events)
  if (Array.isArray(qp.events) && qp.events.length) {
    qp.events.forEach((item) => {
      // 自定义指标
      if (item.customIndex) {
        item.customIndex.forEach((items) => {
          eventNames.push(items.eventName)
        })
      } else {
        eventNames.push(item.eventName)
      }
    })
  } else if (isObject(qp.events)) {
    eventNames = [qp.events.eventName]
  }
  eventNames = uniqBy(eventNames.filter(Boolean)).join(',')*/
